...
- ✅ **Status (2026-08-09) — the Docker jail is BUILT and shipped opt-in** (`aiforum.llm.jail.enabled`, default **false**; with it off, behaviour is byte-identical to the above). `claude -p` now optionally runs in a per-invocation container — read-only rootfs, all capabilities dropped, tmpfs scratch, the credential the only bind mount — on an `--internal` Docker network whose one route out is a squid sidecar filtering by destination hostname, deny-by-default (`api.anthropic.com`, `api.github.com`). Enforcement is topology, not proxy-variable cooperation. **Not covered:** feed/article fetching (runs in-JVM on the host), the openai/opencode providers, and the app itself — so the "treat web-derived input as adversarial" stance above is unchanged. Design, the credential decision, and the decision log: `plan_docs/llm-sandbox.md`; opt-in verification: `./gradlew jailContract`.
