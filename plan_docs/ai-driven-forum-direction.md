...
S-number — "Ambient Slice 5" was already taken by the feed-source slice). Remaining on this map:
**S6**.
...
| 2026-07-26 | **The §6.3 root ships NOW as storage** (`kind='root'`, owner-only via DDL CHECK) **and is injected NEVER this slice** — the recorded owner call | SQLite cannot add a CHECK by ALTER, so a deferred root could never get DDL enforcement — the row ships when the table is born. Injecting now would put two identity sources in one prompt with undecided precedence; prompt identity stays solely the composed `system_prompt`, and a later slice wires injection with its own steer and truncation decisions |
